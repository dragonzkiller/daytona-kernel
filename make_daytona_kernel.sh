#
# Make an daytona kernel - minor config capabilities
#
# !@ acerbix - 2012
#

export ARCH=arm 
export CROSS_COMPILE=~/android/cm9system/prebuilt/linux-x86/toolchain/arm-eabi-4.4.3/bin/arm-eabi-
X_ARG=-j7
PLATFORM=tegra_daytona_android_defconfig

DEFCONFIG=0
DISTCLEAN=0
MENUCONFIG=0
CM9_TOP="~/android/cm9system/"
DO_COPY=0
while getopts :a:c:x:p:n:dm opts; do
	...
	\?)	
		...
		-p <config> - default - tegra_daytona_android_defconfig
...
if [ $DO_COPY -eq 1 ]
	then
		echo "Copyng kernel to ${CM9_TOP}/device/moto/daytona/kernel"
		cp arch/$ARCH/boot/zImage ${CM9_TOP}/device/moto/daytona/kernel
		touch ${CM9_TOP}/device/moto/daytona/kernel
		echo "Copyng modules to ${CM9_TOP}/device/moto/daytona/modules/"
		for mod in `find  .  -name "*.ko" -print `;  
			do   cp  $mod "${CM9_TOP}/device/moto/daytona/modules/" ; done 
fi
